...
//Xử lý tìm kiếm
    ...
    $query_products = "SELECT id, name, price, brand FROM products";
    if ($searchTerm) {
        if ($searchField == 'name') {$query_products .= " WHERE name LIKE '%$searchTerm%'";} else if ($searchField == 'brand') {$query_products .= " WHERE brand LIKE $searchTerm%'";
        }
    }
    $result_products = mysqli_query($con, $query_products);$products = mysqli_fetch_all($result_products, MYSQLI_ASSOC);
