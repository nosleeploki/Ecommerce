...
while ($row = mysqli_fetch_array($result)) {
        $sum += $row["Price"];
        $id = $row["id"] . ", ";
        echo "<tr><td>" . "#" . $row["id"] . "</td><td>" . $row["Name"] . "</td><td>" . $row["Price"]." VND" . "</td><td><a href='cart-remove.php?id={$row['id']}' class='remove_item_link'> Remove</a></td></tr>";
    }
    $id = rtrim($id, ", ");
    echo "<tr><td></td><td>Thành tiền</td><td> " . $sum . " VND". "</td><td><a href='success.php' class='btn btn-primary'>Đặt hàng</a></td></tr>";
    ?>
                            </tbody>
                            <?php
} else {
